...
def draw_masks(ax, img, masks, th_index, area_list, color=None, with_edge=True, alpha=0.8):
    ...
    # scalar 값 매핑
    norm = colors.Normalize(vmin=min(area_list), vmax=max(area_list), clip=True)
    mapper = cm.ScalarMappable(norm=norm, cmap= cm.jet)

    for i, mask in enumerate(exclude_masks):
        value = area_list[i]
        heatmap = np.array(np.array(mapper.to_rgba(value)[:3])*255)
...
def imshow_det_bboxes(img,
    ...
    if segms is not None:
        ...
        draw_masks(ax, img, segms, th_index, area_list, colors, with_edge=True)
