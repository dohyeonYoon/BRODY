...
def Generate_Depthmap_2(filename, mask_list, contour_list, th_index):
    """깊이정보를 받아와서 각 개체를 이루는 mask에 median filter를 적용하고
       mask의 median z값을 contour에 뿌려주는 함수.
    ...
    # Apply median filter to instace mask.
    z_list = [[depth_map[j[0][1], j[0][0]] for j in i] for i in mask_list]
    median_z_list = [int(np.median(i)) for i in z_list]

    # Distribute median z value of mask to the contour.
    for j in th_index:
        for k in contour_list[j]:
            depth_map[k[0][1],k[0][0]] = int(median_z_list[j])

    return depth_map, th_index

def Convert_3D(filename, depth_map):
